...
string prod(string a, string b) {
    ...
    return max<=t? multLarge(a, b) :removeLeadingZeros(addLarge(powLarge(prod(divideLarge(a, max / 2), divideLarge(b, max / 2)), (max / 2) * 2),powLarge(addLarge(prod(divideLarge(a, max / 2), remLarge(b, max / 2)), prod(divideLarge(b, max / 2), remLarge(a, max / 2))), max / 2),prod(remLarge(a, max / 2), remLarge(b, max / 2))));
}
